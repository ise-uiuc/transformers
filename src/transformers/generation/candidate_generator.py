...
class AssistedCandidateGenerator(CandidateGenerator):
    ...
    def get_candidates(
        ...
        if self.acceptance_tracker:
            self.acceptance_tracker.proposed.append(
                int(candidate_ids.shape[-1]) - int(input_ids.shape[-1])
            )
...
class BlazeditCandidateGenerator(AssistedCandidateGenerator):
    def __init__(
        ...
        assert (
            generation_config.max_matching_ngram_size is None
        ), "Do not set max_matching_ngram_size for blazedit"

        assistant_model.generation_config.assistant_confidence_threshold = (
            None  # disabled for pld
        )
        super().__init__(
            input_ids,
            assistant_model,
        ...
        self.max_matching_ngram_size = blazedit_config.get(
